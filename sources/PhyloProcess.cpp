...
void PhyloProcess::PruningAncestral(const Link* from, int site) {
    ...
    for (const Link* link = from->Next(); link != from; link = link->Next()) {
        ...
        try {
            ...
            GetSubMatrix(link->GetBranch()->GetIndex(), site)
                .GetFiniteTimeTransitionProb(state, aux, GetBranchLength(link->GetBranch()->GetIndex()) * GetSiteRate(site));
...
void PhyloProcess::ResampleSub(const Link* from, int site) {
    ...
    if (from->isRoot()) {
        ...
        pathmap[from->GetNode()][site] = SamplePath(GetState(from->Out()->GetNode(), site), GetState(from->GetNode(), site),
...
BranchSitePath* PhyloProcess::ResampleAcceptReject(int maxtrial, int stateup, int statedown, double rate, double totaltime,
...
BranchSitePath* PhyloProcess::ResampleUniformized(int stateup, int statedown, double rate, double totaltime,
                                                  const SubMatrix& matrix) {
...
void PhyloProcess::AddPathSuffStat(PathSuffStat& suffstat) const { RecursiveAddPathSuffStat(GetRoot(), suffstat); }
...
void PhyloProcess::LocalAddPathSuffStat(const Link* from, PathSuffStat& suffstat) const {
    ...
    for (int i = 0; i < GetNsite(); i++) {
        if (missingmap[nodeindex][i] == 2) {
            ...
            pathmap[from->GetNode()][i]->AddPathSuffStat(suffstat,
...
void PhyloProcess::RecursiveAddPathSuffStat(const Link* from, BidimArray<PathSuffStat>& suffstatarray,
                                            const BranchAllocationSystem& branchalloc) const {
    if (from->isRoot()) {
...
void PhyloProcess::LocalAddPathSuffStat(const Link* from, BidimArray<PathSuffStat>& suffstatarray, int cond) const {
    ...
    for (int i = 0; i < GetNsite(); i++) {
        if (missingmap[nodeindex][i] == 2) {
            ...
            pathmap[from->GetNode()][i]->AddPathSuffStat(suffstatarray(cond, i),
                                                         GetBranchLength(from->GetBranch()->GetIndex()) * GetSiteRate(i));
        }
    }
}
...
void PhyloProcess::LocalAddPathSuffStat(const Link* from, Array<PathSuffStat>& suffstatarray) const {
    ...
    for (int i = 0; i < GetNsite(); i++) {
        if (missingmap[nodeindex][i] == 2) {
            ...
            pathmap[from->GetNode()][i]->AddPathSuffStat(suffstatarray[i],
                                                         GetBranchLength(from->GetBranch()->GetIndex()) * GetSiteRate(i));
        }
    }
}
...
void PhyloProcess::RecursiveAddLengthSuffStat(const Link* from,
...
void PhyloProcess::LocalAddLengthSuffStat(const Link* link, PoissonSuffStat& suffstat) const {
    ...
    for (int i = 0; i < GetNsite(); i++) {
        if (missingmap[nodeindex][i] == 1) {
            pathmap[link->GetNode()][i]->AddLengthSuffStat(suffstat, GetSiteRate(i),
