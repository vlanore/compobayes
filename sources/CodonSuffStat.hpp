...
/**
 * \brief A sufficient statistic for substitution histories, as a function of the underlying nucleotide rate matrix (for
 * Muse-Gaut codon models only)
 *
 * The generic sufficient statistics for substitution histories (S) as a function of the rate matrix Q are defined in
 * PathSuffStat.
 ...
 *
 * NucPathSuffStat implements this idea, and provides methods for calculating these suffstats based on generic PathSuffStat
 * at the codon-level,
...
class NucPathSuffStat : public SuffStat, public tc::Component {
  public:
    ...
    //! \brief compute the 4x4 path suff stats out of 61x61 codonpathsuffstats over a one-dim array, and sum them up into
    //! this
    //!
    ...
    //!
    //! The codon state space is given as an argument (the nucleotide matrix or the suff stat themselves do not know the
    //! genetic code)
 ...
 * p(S | omega) propto omega^count exp(-beta * omega), where count (integer) and beta (positive real number) are the suff
 * stats.
 * Note that this is in fact analogous to a Poisson distribution, with mean omega, and thus, OmegaPathSuffStat derives from
 * PoissonSuffStat.
 */
...
class OmegaPathSuffStat : public PoissonSuffStat /*, public tc::Component*/ {
  public:
    ...
    //!
    //! note that omega suff stat depends on the other aspects of the codon matrix (in particular, the nucleotide rate
    //! matrix)
